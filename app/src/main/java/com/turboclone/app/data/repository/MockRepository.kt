...
object MockRepository {
    ...
    // طلبات انضمام السائقين (وهمية بالكامل، تُخزَّن في الذاكرة فقط)
    val driverApplications = mutableStateListOf<DriverApplication>()

    fun submitDriverApplication(app: DriverApplication) {
        driverApplications.add(0, app)
    }

    fun approveDriver(id: String) {
        val i = driverApplications.indexOfFirst { it.id == id }
        if (i != -1) driverApplications[i] = driverApplications[i].copy(status = DriverApplicationStatus.APPROVED)
    }

    fun rejectDriver(id: String) {
        val i = driverApplications.indexOfFirst { it.id == id }
        if (i != -1) driverApplications[i] = driverApplications[i].copy(status = DriverApplicationStatus.REJECTED)
    }
    ...
    fun generateNearbyDrivers(centerLat: Double, centerLng: Double, count: Int = 5): List<Driver> {
        val names = listOf("عبدالله المبروك", "سالم الزوي", "ناصر الككلي", "مفتاح سعد", "فيصل خليفة", "بندر عمر")
        ...
        return (0 until count).map { i ->
            ...
            Driver(
                ...
                plateNumber = "${Random.nextInt(1000, 9999)} ليبيا",
